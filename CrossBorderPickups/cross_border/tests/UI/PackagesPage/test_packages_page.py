...
from CrossBorderPickups.cross_border.page_objects.UI.PackagesPage.packages_page import PackagesPage, PackagesList, \
    CreateOrderModal, AddContentModal
...
class TestPackagesPage(BaseTest):
    ...
    def test_visibility_of_create_order_link_unlink_discard_buttons(self):
        ...
        for element in enable_disable_buttons:
            ...
            for p_id in package_ids_to_select:
                package_list.select_package_by_id(package_id=p_id)
    ...
    def test_create_order_modal_shows_only_pending_order_creation_packages_id(self):
        ...
        package_ids_from_create_modal = package_list.get_all_packages_id()
    ...
    def test_visibility_of_fields_under_create_order_modal(self, package_receive_method):
        ...
        if package_receive_method == create_order_modal_constant.PACKAGE_RECEIVE_BY_MAIL:
            ...
            assert create_order.is_element_visible(by_locator=package_pickup_location_element), \
                "Package pickup location radio button for '{}' location is missing under 'Order Details' panel.".format(
                    pickup_location)
